...
class dtree:                       # decision tree class
    ...
    def classify(self, tree, datapoint):      # use tree to classify pattern datapoint
        if type(tree) == type("string"):   # have reached a leaf
            ...
            a = list(tree.keys())[0]         # first node in tree
    ...
    def make_tree(self, data, classes, featureNames, maxlevel=-1, level=0, forest=0):
        ...
        # newClasses = unique class names; e.g., ['y', 'n']?
        ...
        for aclass in newClasses:  # for each unique output class
            frequency[index] = classes.count(
                aclass)  # num times aclass in targets
        ...
        #	return classes[0]                        # BAD? ******
            ...
            featureSet = list(range(nFeatures))  # indices of features (input atrs)
            if forest != 0:
            ...
            # input feature with highest info gain
            bestFeature = np.argmax(gain)
            # start dict representing tree
    ...
    def printTree(self, tree, spacing, out):     # display indented tree
        fid = self.fid                   # fid = output file, spacing = indentation
        if type(tree) == dict:           # if it's a tree/subtree
            fid.write("{} {}?".format(
                spacing, list(tree.keys())[0]) + "\n")  # atr name
            for item in list(tree.values())[0].keys():          # for each value of atr
                # print spacing, item
                fid.write("{} {}".format(spacing, item) +
                          "\n")         # write val name
                # write subtrees
                self.nodes += 1
                self.printTree(list(tree.values())[0][item], spacing + "\t", False)
        else:
            # write classif
            fid.write("{} {} {}".format(spacing, "\t->  ", tree) + "\n")
            self.leaves += 1
        if out is True:
            fid.write("Number of nodes = " + str(self.nodes) + "\n")
            fid.write("Number of leaves = " + str(self.leaves) + "\n")
        return
